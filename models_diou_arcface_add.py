...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nC, nID, nE, img_size, yolo_layer):
        ...
        self.emb_scale = math.sqrt(2) * math.log(self.nID-1) if self.nID>1 else 1  # 为啥？
    ...
    def forward(self, p_cat,  img_size, targets=None, classifier=None, test_emb=False):
        ...
        if targets is not None:
            ...
            # tconf[b][fg_index] = 1, tconf[b][bg_index] = 0, tconf[b][ign_index] = -1
            tconf, tbox, tids = tconf.cuda(), tbox.cuda(), tids.cuda()
            mask = tconf > 0  # 把前景mask出来  # (nB, nA, nGh, nGw) # tconf为fg=1，bg=-1, ignore=0
            ...
            if nM > 0:
                # lbox0 = self.SmoothL1Loss(p_box[mask], tbox[mask])
                """ ************ DiouLoss ************* """
                """ tbox(encode_delta): [dx, dy, dw, dh]. 其中，encode_delta: dx = (gx - px) / pw, dw = torch.log(gw/pw) """
                # TODO, 确认原来有没有batch维度
                decode_p_box = decode_delta_map(p_box, self.anchor_vec.to(p))  # p_box: torch.Size([1, 4, 30, 54, 4])
                decode_tbox = decode_delta_map(tbox, self.anchor_vec.to(p))   # tbox: torch.Size([1, 4, 30, 54, 4])
                lbox = 1- bbox_diou(decode_p_box[mask], decode_tbox[mask]) 
                lbox = torch.mean(lbox)  # ret = torch.mean(ret) if reduction == 'mean' else torch.sum(ret)      
            else:
            ...
            emb_mask,_ = mask.max(1)  # emb_mask (nB, nGh, nGw)
            
            # For convenience we use max(1) to decide the id, TODO: more reseanable strategy
            tids,_ = tids.max(1) 
            tids = tids[emb_mask]
            embedding = p_emb[emb_mask].contiguous()
            # embedding = self.emb_scale * F.normalize(embedding) # TODO，这里先删掉
            nI = emb_mask.sum().float()
            
            """ TODO，如果改成arcface这部分需要怎么改？这一步要继续看 """
            if  test_emb:
                if np.prod(embedding.shape)==0  or np.prod(tids.shape) == 0:
                    return torch.zeros(0, self.emb_dim+1).cuda()
                # embedding = F.normalize(embedding)  # 从前面移到这里来了，如果不要scale呢？
                embedding = self.emb_scale * F.normalize(embedding)  # 从前面移到这里来了
                emb_and_gt = torch.cat([embedding, tids.float()], dim=1)
                return emb_and_gt
            
            """ ********* 特征提取 feature ******** """
            if len(embedding) > 1:
                # classifier0 = nn.Linear(self.emb_dim, self.nID).cuda() if self.nID > 0 else None
                # # logits = classifier0(embedding).contiguous().cuda()  # 这边换   # 这里： self.classifier = nn.Linear(self.emb_dim, nID) if nID>0 else None
                # logits = classifier0(embedding)  # 这边换   # 这里： self.classifier = nn.Linear(self.emb_dim, nID) if nID>0 else None
                # lid0 =  self.IDLoss(logits, tids.squeeze())   # self.IDLoss = nn.CrossEntropyLoss(ignore_index=-1)
                # print("******** tids *************", tids)
                # TODO,直接tids这样作为label可以吗？ metric_fc(feature, label)
                import copy
                tids_pos = copy.deepcopy(tids)
                tids_pos[tids_pos==-1]=0 
                output = classifier(embedding, tids_pos.squeeze())  # output = (one_hot * phi) + ((1.0 - one_hot) * cosine)，# 这里传label主要是用于分类内、类间的
                lid = self.IDLoss(output, tids.squeeze())
                # print ("lid0", lid0.item(), "lid", lid.item())

            # Sum loss components
            # loss = torch.exp(-self.s_r)*lbox + torch.exp(-self.s_c)*lconf + torch.exp(-self.s_id)*lid + \
            #        (self.s_r + self.s_c + self.s_id)
            loss =lbox + lconf + lid
            loss *= 0.5  # 为啥我当时要把这句话注释掉
            # print (loss, loss.item(), lbox.item(), lconf.item(), lid.item(), nT)
            return loss, loss.item(), lbox.item(), lconf.item(), lid.item(), nT
            # return loss, self.s_r, self.s_c, self.s_id, loss.item(), lbox.item(), lconf.item(), lid.item(),  nT 
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg_dict, nID=0, test_emb=False):
        ...
        assert nID>0, "nID is less than 1"
        metric = 'add_margin'  #'arc_margin'  # 搞个默认
        if metric == 'add_margin':
            self.classifier = AddMarginProduct(self.emb_dim, nID, s=30, m=0.35)
        elif metric == 'arc_margin':
            # asy_margin = False
            self.classifier = ArcMarginProduct(self.emb_dim, nID, s=30, m=0.5, easy_margin=False)
        elif metric == 'sphere':
            self.classifier = SphereProduct(self.emb_dim, nID, m=4)
        else:
            # TODO
            # RuntimeError: Error(s) in loading state_dict for Darknet:
            # Unexpected key(s) in state_dict: "classifier.bias".
            print("Please specific the classifier...")
            # self.classifier = nn.Linear(self.emb_dim, nID)  # 输入输出不对应
...
def create_grids(self, img_size, nGh, nGw):
    self.stride = img_size[0]/nGw  # self.stride：每个grid的步长
    ...
    # build wh gains
    self.anchor_vec = self.anchors / self.stride  # self.anchors在cfg中设置，是原图的大小，self.anchor_vec是相对于格子的坐标
