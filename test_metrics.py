        ...
        for si, (labels, detections) in enumerate(zip(targets, output)):
            ...
            if labels.size(0) == 0:
                ...
                for *pred_bbox, conf, obj_conf  in detections:
                    ...
                    iou = bbox_iou(pred_bbox, target_boxes, x1y1x2y2=True)[0]   # mAP@IoU
        ...
        for si, (labels, detections) in enumerate(zip(targets, output)):
            ...
            if labels.size(0) == 0:
                ...
                for *pred_bbox, conf, obj_conf  in detections:
                    ...
                    # TODO, mAP@GIoU
                    # Compute iou with target boxes
                    iou = bbox_giou(pred_bbox, target_boxes, x1y1x2y2=True)[0]  # 为了简便，现在还是用iou，之后要改为giou
                    # Extract index of largest overlap
                    best_i = np.argmax(iou)
                    # If overlap exceeds threshold and classification is correct mark as correct
                    if iou[best_i] > iou_thres and obj_pred == labels[best_i, 0] and best_i not in detected:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iou-thres', type=float, default=0.5, help='iou threshold required to qualify as detected')   # 0.75
